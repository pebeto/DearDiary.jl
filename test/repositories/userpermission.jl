    ...
    @testset verbose = true "user permission repository" begin
        @testset verbose = true "insert" begin
            ...
            @testset "insert duplicate user permission" begin
                @test TrackingAPI.insert(TrackingAPI.UserPermission, user.id, project_id) isa Tuple{Nothing,TrackingAPI.Duplicate}
            end
        end
        ...
        @testset verbose = true "fetch" begin
            ...
            @testset "fetch with non-existing user" begin
                userpermission = TrackingAPI.fetch(TrackingAPI.UserPermission, 9999, project_id)

                @test userpermission |> isnothing
            end

            @testset "fetch with non-existing project" begin
                userpermission = TrackingAPI.fetch(TrackingAPI.UserPermission, user.id, 9999)
        ...
        @testset verbose = true "delete" begin
            ...
            @testset verbose = true "delete using userpermission id" begin
                ...
                @test TrackingAPI.fetch(TrackingAPI.UserPermission, user.id, project_id) |> isnothing
            end
            ...
            @testset verbose = true "delete using project" begin
                ...
                @test TrackingAPI.fetch(TrackingAPI.UserPermission, user.id, project.id) |> isnothing
            end
